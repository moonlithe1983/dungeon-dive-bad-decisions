...
| `2.2 Product Health` | `not yet` | Retention, conversion, and live-ops health foundations are not yet proven or fully built. | `src/analytics/schema.ts`, `src/analytics/client.ts` |
| `2.3 Technical Quality` | `partial` | Resume/restart/readability foundations are strong, but target-device performance and validated analytics are still incomplete. | `src/save/runRepo.ts`, `app/_layout.tsx`, `src/hooks/use-responsive-layout.ts` |
...
| `4.3 Short-Term Fun + Long-Term Goals` | `partial` | Runs and meta goals are present, but long-tail content breadth is still limited. | `app/index.tsx`, `app/hub.tsx`, `app/progression.tsx` |
...
| `5.5 Meta Loop` | `partial` | There is visible next-goal and persistent progression, but daily/weekly goal structure is not built. | `src/progression/next-goal.ts`, `app/hub.tsx`, `app/progression.tsx` |
| `5.6 Seasonal / Live-Ops Loop` | `not yet` | Seasonal and live-ops structures are not implemented. | `n/a` |
...
| Long goals are decomposed into sub-goals. | `partial` | Next-goal surfacing exists, but richer quest/subgoal decomposition is not yet built. | `src/progression/next-goal.ts`, `app/progression.tsx` |
...
| Analytics instrumentation is present and validated. | `partial` | Schema and client are present, key events are wired, and remote validation probes can now be sent from the dev smoke screen, but a real production back-end still needs confirmation. | `src/analytics/schema.ts`, `src/analytics/client.ts`, `src/analytics/http-adapter.ts`, `app/dev-smoke.tsx`, `app/onboarding.tsx`, `app/settings.tsx`, `src/state/runStore.ts`, `app/run-map.tsx`, `app/battle.tsx`, `app/reward.tsx`, `app/event.tsx` |
...
3. Analytics can now validate against a generic remote endpoint, but a real production pipeline still needs to be chosen and proven.
4. Monetization and live-ops sections remain intentionally unimplemented.
